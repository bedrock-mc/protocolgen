...
type ClientCacheStatus struct {
	// IsCacheSupported specifies if the blob cache is enabled. If false, the server should not attempt
	// to use the blob cache. If true, it may do so, but it may also choose not to use it.

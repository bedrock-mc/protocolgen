...
| 50 | `InventorySlotPacket` | Wire conflict: the canonical Container Id is a fixed u8 while the protocol 2193 InventorySlot marshal writes WindowID as var_u32 (identical below 128). ItemInstance extra-data is decoded from one byte buffer and its canBePlacedOn/canBreak arrays are parsed inside it, so the extractor counts them as outer operations where the canonical manifest has one opaque user-data buffer. |
...
| 315 | `ServerboundDiagnosticsPacket` | Open source conflict: Endstone 1.26.51.1 marks the entity diagnostic Position and Dimension optional and the canonical manifest carries that selection, while the 1.26.51 Mojang schema lists both as required and the protocol 2193 fork writes them unconditionally. |

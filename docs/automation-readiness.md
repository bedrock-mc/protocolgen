...
The `.51` manifest compared with gophertunnel `8d903c5192f794ca33fea2d84f7afec3e3393bc5` has **204 agreements, 27 reviewed divergences, and no unresolved comparisons**. The protocol 2168 run against `be6713da` (199 agreements, 29 divergences) remains reproducible with that revision's lock and baseline from git history; the helper summaries are reviewed for both revisions. The reviewed divergences include extractor limitations and framing differences; they are not proved vanilla bugs. No disputed codec was changed merely to satisfy this oracle.
...
| 324 | `PlayerVideoCapturePacket` | Wire conflict: both 1.26.51 sources and LeviLamina order the action union start then stop, so start is selector 0. The protocol 2193 PlayerVideoCapture marshal numbers stop as 0 and start as 1, so it swaps the two actions. The fork is the suspect side; its u8 action and the var_u32 selector are the same byte for these values. |

...
### Divergences awaiting a manifest correction

The pinned gophertunnel and CloudburstMC/Protocol agree with each other against the manifest on these, so they are not comparison debt and are deliberately not in the accepted baseline; `verify-gophertunnel` fails until the manifest is corrected through the evidenced correction path.
